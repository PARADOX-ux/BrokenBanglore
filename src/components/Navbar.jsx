...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] bg-cream border-b-2 border-black w-full h-16 md:h-20 flex items-center justify-between px-4 md:px-8">
      <Link to="/" className="font-body font-black text-lg md:text-xl flex items-center gap-1 text-black tracking-tight">
        Broken Bengaluru
      </Link>
      
      {/* Desktop Nav */}
      <nav className="hidden lg:flex items-center gap-8 font-black text-xs md:text-sm uppercase tracking-widest text-black/60">
        {navLinks.map((link) => (
          <Link 
            key={link.path} 
            to={link.path}
